...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#history-task-title').innerText(), '项目');
    assert.deepEqual(await page.locator('[data-history-tab]').allTextContents(), ['全部', '自动任务', '共享项目']);
    assert.equal(await page.locator('[data-team-action="share"]').innerText(), '发布项目 Session');
    assert.equal(await page.locator('[data-team-folder="team-demo"][data-knowledge-folder="团队产物"]').count(), 0);
    assert.equal(await page.locator('[data-team-shared-history="true"] .history-time').innerText(), '接力共享');
    assert.equal(await page.locator('[data-team-shared-history="true"] [data-team-shared-open]').innerText(), '接力项目');
    assert.deepEqual(await page.locator('#knowledge-table-head > span').allTextContents(), ['', '项目信息', '共享人', '类型', '状态', '更新时间', '操作']);
    assert.equal(await page.locator('#knowledge-header-actions').isHidden(), true);
    assert.match(await rows.first().innerText(), /4 项上下文/);
    assert.match(await rows.first().innerText(), /来源 Agent：/);
    ...
    assert.equal(await page.locator('[data-team-shared-history="true"] [data-team-shared-open]').innerText(), '查看项目');
    await page.reload();
    await openProjects();
    ...
    await openProjects();
    assert.equal(await rows.count(), 0);
    await page.locator('[data-team-action="share"]').click();
    assert.equal(await page.locator('#team-share-publish').innerText(), '确认共享项目并发布');
    await page.locator('#team-share-publish').click();
    assert.match(await page.locator('#team-share-result').innerText(), /已发布到 产品共创组 · 团队共享项目/);
    await page.locator('#team-share-enter').click();
    assert.equal(await page.locator('#knowledge-folder-title').innerText(), '团队共享项目');
    assert.equal(await page.locator('[data-team-tree-toggle="team-demo"]').getAttribute('aria-expanded'), 'true');
    assert.equal(await page.locator('#knowledge-header-actions').isHidden(), true);
    assert.deepEqual(await page.locator('#knowledge-table-head > span').allTextContents(), ['', '项目信息', '共享人', '类型', '状态', '更新时间', '操作']);
    assert.equal(await rows.count(), 1);
    assert.match(await rows.first().innerText(), /来源 Agent：销售简报 Agent/);
    assert.deepEqual(errors, []);
    console.log('PASS: aligned shared-project columns; project-session publishing chain; single relay entry; combined relay settings; read/edit permissions; source Agent and own Agent handoff actions; delete.');
  } finally {
    await browser.close();
  }
