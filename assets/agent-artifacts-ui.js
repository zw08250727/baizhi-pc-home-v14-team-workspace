...
window.ArtifactUI = (() => {
  ...
  function sync() {
    ...
    ['personal','enterprise'].forEach(space => {
      ...
      q(`#artifact-tree-${space}`).innerHTML = `<div class="knowledge-tree-node artifact-tree-group"><button class="tree-folder-toggle artifact-root" data-artifact-folder="${model.folder(space)}" aria-expanded="true"><svg class="icon tree-chevron"><use href="#ico-chevron"/></svg><svg class="icon tree-folder-icon"><use href="#ico-folder"/></svg><span class="tree-folder-name">Agent 产物</span><span class="artifact-system">系统</span></button><div class="knowledge-tree-children artifact-tree-children">${agents.map(agent => `<button class="side-sub-item knowledge-leaf artifact-leaf" data-artifact-folder="${esc(model.folder(space,agent))}"><svg class="icon"><use href="#ico-folder"/></svg><span class="tree-folder-name">${esc(agent)}</span></button>`).join('')}</div></div>`;
    });
  }
  function init() {
