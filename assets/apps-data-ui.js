window.AppsDataUI = (() => {
  ...
  function renderList() {
    ...
    const copy=team()?'团队成员可只读查看当前团队应用数据；表 Owner、团队创建人或授权管理员可管理权限，数据由 Agent 写入。':M.apps.some(a=>M.canManage(state.user,a,context()))?'展示已使用及有权管理的应用':'仅展示我的数据，成功更新后自动加入应用';
    root.innerHTML=head('应用数据','查看 Agent 更新的业务数据与变更记录')+`<div class="apps-toolbar"><div class="apps-filter-group"><span class="apps-filter-copy">${copy}</span></div><label class="apps-search"><svg class="icon"><use href="#ico-search"/></svg><input type="search" data-app-filter="search" aria-label="搜索应用" placeholder="搜索应用名称" value="${esc(state.search)}"></label></div><div class="apps-panel">${list.length?`<div class="apps-table-scroll"><table class="apps-table"><thead><tr><th>应用名称</th><th>数据表</th><th>数据条数</th><th>最近更新时间</th><th>最近更新人</th></tr></thead><tbody>${list.map(a=>{
      const scope=initialScope(a),h=M.history(state.user,a.id,scope,context()),total=a.tables.reduce((n,t)=>n+M.rows(state.user,a.id,t.id,scope,context()).length,0);
  ...
  function filteredTableRows(a,table) {
    ...
    const rows=all.filter(r=>(!state.owner||r.owner===state.owner)&&(!state.department||M.users[r.owner]?.department===state.department)&&(!state.search||r.values.some(v=>String(v).toLowerCase().includes(state.search.toLowerCase()))));
    return {all,rows};
  }
  ...
  function renderDetail() {
    ...
    const ownerIds=[...new Set(state.tab==='history'?fullHistory.map(r=>r.actor):M.rows(state.user,a.id,table.id,state.scope,context()).map(r=>r.owner))];
    const ownerFilter=wideScope?`<select aria-label="${state.tab==='history'?'触发人':'数据所属人'}" data-app-filter="owner">${opts([['',state.tab==='history'?'全部触发人':'全部所属人'],...ownerIds.map(id=>[id,M.users[id]?.name||id])],state.owner)}</select>`:'';
    if(state.tab==='data'){
      ...
      body=`<div class="apps-data-layout">${tableNav}<section class="apps-sheet-content" aria-label="${esc(table.name)}"><div class="apps-sheet-heading"><div class="apps-sheet-heading-copy"><strong>${esc(table.name)}</strong><span class="apps-caption">用户只读 · Agent 写入</span></div><div class="apps-sheet-actions"><button class="apps-btn apps-download-btn" type="button" data-app-download title="下载当前权限和筛选条件下的全部记录"><svg class="icon" aria-hidden="true"><use href="#ico-download"/></svg><span>下载表格</span></button></div></div><div class="apps-toolbar"><div class="apps-filter-group">${ownerFilter}${wideScope?`<select data-app-filter="department" aria-label="部门">${opts([['','全部部门'],...[...new Set(all.map(r=>M.users[r.owner]?.department).filter(Boolean))].map(d=>[d,d])],state.department)}</select>`:'<span class="apps-filter-copy">数据由已授权 Agent 持续更新</span>'}</div><label class="apps-search"><svg class="icon"><use href="#ico-search"/></svg><input type="search" aria-label="搜索表格数据" data-app-filter="search" placeholder="搜索表格内容" value="${esc(state.search)}"></label></div><div class="apps-table-scroll apps-grid-scroll"><table class="apps-table apps-data-grid" style="width:${44+widths.reduce((n,w)=>n+w,0)+170+158}px"><colgroup><col style="width:44px">${widths.map(w=>`<col style="width:${w}px">`).join('')}<col style="width:170px"><col style="width:158px"></colgroup><thead><tr><th scope="col">#</th>${table.fields.map(f=>`<th scope="col">${esc(f[0])}<small>${esc(f[1])}</small></th>`).join('')}<th scope="col">数据所属人</th><th scope="col">更新时间</th></tr></thead><tbody>${slice.map((r,i)=>`<tr><td class="apps-readonly-cell" aria-readonly="true">${(state.page-1)*7+i+1}</td>${r.values.map((v,j)=>`<td class="apps-readonly-cell ${/数值|整数/.test(table.fields[j]?.[1])?'apps-number-cell':''}" aria-readonly="true">${value(v)}</td>`).join('')}<td class="apps-readonly-cell" aria-readonly="true">${person(r.owner)}</td><td class="apps-time-cell apps-readonly-cell" aria-readonly="true">${esc(r.updated)}</td></tr>`).join('')}</tbody></table>${!rows.length?empty('当前范围暂无记录',state.search||state.owner||state.department?'请调整筛选条件':'Agent 更新这张表后，可在这里查看。'):''}</div>${footer}</section></div>`;
    }else{
